...
impl<'a> Iterator for ReserveEntitiesIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(&id) = self.id_iter.next() {
            // We had previously reserved an item in the freelist.
            let entity = Entity {
                generation: self.meta[id as usize].generation,
                id,
            };
            Some(entity)
        } else if let Some(id) = self.id_range.next() {
            // The freelist ran out, so we reserved a new ID.
            let entity = Entity { generation: 0, id };
            Some(entity)
        } else {
            None
        }
...
impl Entities {
    ...
    ///
    /// Locations can be determined by the return value of `flush_one_reservation` and should all be
    /// written immediately after flushing.
    pub fn reserve_entities(&self, count: u32) -> ReserveEntitiesIterator {
        ...
        let freelist_start = cmp::max(range_start, 0) as usize;
        let freelist_end = cmp::max(range_end, 0) as usize;
        ...
        ReserveEntitiesIterator {
            meta: &self.meta[..],
            id_iter: self.pending[freelist_start..freelist_end].iter(),
    ...
    pub fn reserve_entity(&self) -> Entity {
        ...
        if n > 0 {
            ...
            // Grab a new ID, outside the range of `meta.len()`. `flush_one_reservation()` must
    ...
    /// Check that we do not have pending work requiring `flush_one()` to be called.
    ...
    /// Allocate a specific entity ID and generation.  May return the location of the entity
    /// previously allocated to the specified ID.
    ///
    /// Location should be written immediately.
    pub fn alloc_at(&mut self, entity: Entity) -> Option<Location> {
        ...
        let loc = if entity.id as usize >= self.meta.len() {
            ...
            .pending
            .iter()
            .find(|item| **item == entity.id)
            .copied()
        {
            self.pending.swap_remove(index as usize);
    ...
    pub fn reserve(&mut self, additional: u32) {
        ...
        let freelist_size = cmp::max(self.free_cursor.load(Ordering::Relaxed), 0);
    ...
    /// Performs one deferred EntityMeta allocation, returning the new ID and `EntityMeta`.
    ///
    /// This must be called repeatedly until it returns None after any reserve_* calls.
